...
add_action('rest_api_init', function () {
  register_rest_route('jott/v1', '/chat', [
    'methods'  => 'POST',
    'callback' => 'jott_chat_handler',
    'permission_callback' => '__return_true',
  ]);

  register_rest_route('jott/v1', '/history', [
    'methods'  => 'GET',
    'callback' => 'jott_history_handler',
    'permission_callback' => '__return_true',
  ]);
});


function jott_history_handler(WP_REST_Request $request) {
  ...
  $history = jott_load_history($session_id, $conversation_id, $limit);
...
function jott_get_session_id($data) {
...
function jott_load_history($session_id, $conversation_id = 'default', $limit = 20) {
  global $wpdb;
  $table = $wpdb->prefix . 'jott_chat_history';
...
function jott_insert_turn($session_id, $conversation_id, $role, $content) {
  global $wpdb;
  $table = $wpdb->prefix . 'jott_chat_history';
...
function jott_call_n8n($payload) {
  // Put your n8n production webhook URL here:
  // $n8n_url = 'https://YOUR_N8N_DOMAIN/webhook/jott';
   $n8n_url = 'http://127.0.0.1:5678/webhook/jott2026';
  // $n8n_url = 'https://n8n.gregoriopirolini.com/jott';  
...
function jott_chat_handler(WP_REST_Request $request) {
  ...
  $session_id = jott_get_session_id($data);
  $conversation_id = isset($data['conversation_id']) ? sanitize_text_field($data['conversation_id']) : 'default';

  // Load last N messages
  $history = jott_load_history($session_id, $conversation_id, 20);

  // Save user message first
  jott_insert_turn($session_id, $conversation_id, 'user', $message);
  ...
  jott_insert_turn($session_id, $conversation_id, 'assistant', $reply);
